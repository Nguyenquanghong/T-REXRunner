...
//Bird
    Enemy:: Enemy(){
        ...
        birdRect.y = (rand() % 2 == 0) ? 120 : 250;  // Random vị trí cao hoặc thấp
    ...
    void Enemy::updateObjectPosition(SDL_Renderer* renderer) {
        if (isBirdActive) {
            // 🦜 Bird di chuyển
            birdRect.x -= speed * GameManager::speedMultiplier;
            ...
            // Nếu Cactus rời khỏi màn hình, reset vị trí và quay lại Bird
            if (objectRects[currentObjectIndex].x + objectRects[currentObjectIndex].w < 0) {
                isBirdActive = true;  // Quay lại Bird
